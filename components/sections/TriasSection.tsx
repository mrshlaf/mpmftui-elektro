      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {triasItems.map((item, idx) => (
            <TriasCard key={item.title} item={item} idx={idx} />
          ))}
        </div>
      </div>
    </section>
...
function TriasCard({ item, idx }: any) {
        ...
        <div className="h-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-10 rounded-[3rem] transition-all duration-700 hover:border-primary/40 hover:shadow-[0_40px_100px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_40px_100px_rgba(0,0,0,0.4)] flex flex-col justify-between relative shadow-sm dark:shadow-none translate-z-10">
          ...
          <div className="relative z-10" style={{ transform: "translateZ(50px)" }}>
            <div className="flex items-center justify-between mb-10">
              <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-all duration-500 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm ${
                item.color === "blue" ? "text-blue-500" : 
                item.color === "yellow" ? "text-yellow-600" : 
                "text-purple-500"
              } group-hover:scale-110 group-hover:bg-slate-950 dark:group-hover:bg-slate-50 group-hover:text-white dark:group-hover:text-slate-950 group-hover:rotate-12`}>
            ...
            <h3 className="text-3xl font-black text-slate-950 dark:text-white mb-5 tracking-tight group-hover:text-primary transition-colors">
              {item.title}
            </h3>
          ...
          <div className="flex items-center text-primary text-[11px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all duration-500 relative z-10" style={{ transform: "translateZ(30px)" }}>
