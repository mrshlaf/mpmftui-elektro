      ...
      <div className="bg-white border border-slate-200 rounded-[3rem] p-10 md:p-14 shadow-[0_20px_60px_rgba(0,0,0,0.04)] relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex items-center space-x-3 text-blue-600 mb-6">
            ...
            <span className="font-black tracking-[0.2em] text-[10px] uppercase text-slate-600">Legislative Documentation</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter mb-10">Archive New Document</h1>

          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-700 uppercase tracking-widest ml-1">Document Title</label>
              <Input name="title" required placeholder="Judul UU / TAP / Siaran Pers..." className="h-14 rounded-2xl bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:ring-blue-500/20 focus:border-blue-500 px-6" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-700 uppercase tracking-widest ml-1">Document Type</label>
                <select 
                  name="documentType" 
              ...
              <div className="space-y-3">
                ...
                <Input name="category" placeholder="E.g. Internal, Eksternal, Mahasiswa" required className="h-14 rounded-2xl bg-slate-50 border-slate-200 text-slate-900 focus:ring-blue-500/20 focus:border-blue-500 px-6" />
              </div>
            </div>

            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-700 uppercase tracking-widest ml-1">Document Link (Drive/PDF)</label>
              <Input name="linkUrl" required placeholder="https://drive.google.com/..." className="h-14 rounded-2xl bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:ring-blue-500/20 focus:border-blue-500 px-6" />
            </div>

            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-700 uppercase tracking-widest ml-1">Abstrak / Deskripsi Singkat</label>
              <Textarea 
                name="description" 
                required 
